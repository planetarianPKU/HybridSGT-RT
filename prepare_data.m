...
%   Representation Theorem on the five boundary surfaces of the
...
%%
%% ------------------------------------------------------------------------
% Boundary definitions (xmin, xmax, ymin, ymax, zbottom)
%          pxmin, pxmax, pymin, pymax, pzbottom
        %           Bymax
        % ---------------------------------
%   Bxmin |                               | Bxmax
        % |                               |
        % |           Bz                  |    z-axis ↑ (out of plane)
        % |                               |
        % |                               |
        % ---------------------------------
        %           Bymin
...
%% ------------------------------------------------------------------------
% OUT_MESH coordinate arrays (receiver-side boundary geometry)
% We collect all x,y,z coordinates belonging to the boundary surfaces.
% ------------------------------------------------------------------------
X_all_2_check = [];
Y_all_2_check = [];
Z_all_2_check = [];
pall_uni_OUT_all = [];     % store OUT boundary point indices
mat_2d_all = [];


% ------------------------------------------------------------------------
% Locate OUT_MESH coordinate binary files:
%   proc* x_boundary_size_is*.bin  → x coordinates on a boundary
%   proc* y_boundary_size_is*.bin  → y coordinates
%   proc* z_boundary_size_is*.bin  → z coordinates
% Each file stores coordinates for one MPI rank.
% ------------------------------------------------------------------------
...
%% ------------------------------------------------------------------------
% Loop over OUT_MESH files and read x,y,z coordinate arrays
% ------------------------------------------------------------------------
for iname = 1:length(filesx)
    ...
    %% ------------------------------------------------------------
    % Select OUT_MESH boundary nodes for each face:
    %   x = xmin, x = xmax, y = ymin, y = ymax, z = bottom
    % ------------------------------------------------------------
    ...
    % Filter each candidate list so that its other coordinates are inside domain
    ...
    %% ------------------------------------------------------------
    % Combine all OUT-side boundary points for this rank
    % ------------------------------------------------------------
    ...
    % Store coordinates for later matching with IN-side mesh
...
%% ------------------------------------------------------------------------
% IN_MESH geometry, GLL weights, Jacobian, and material parameters
% We extract:
%   • x/y/z coordinates
%   • Jacobian values
%   • Inverse-Jacobian components (∂ξ/∂x, ∂η/∂y, ...)
%   • GLL quadrature weights (wx, wy, wz)
%   • Material parameters (kappa, mu, rho)
%
% These are used for:
%   • Building Jacobians for surface integration
%   • Performing traction calculations
% For details, please see get_GLLintegral.m
% -----------------
X_all_IN_check = [];
Y_all_IN_check = [];
Z_all_IN_check = [];

pall_uni_IN_all = []; % all IN-side boundary node indices
mat_2d_IN_all = [];

% Containers for geometric parameters
jacob_all_IN_check = [];
xixl_all_IN_check = [];
xiyl_all_IN_check = [];
xizl_all_IN_check = [];
...
% ------------------------------------------------------------------------
% Get all IN_MESH data file lists (same structure as OUT_MESH)
% However, they may not use the same parallel computation, so
% points matching is required
% ------------------------------------------------------------------------
...
%% ------------------------------------------------------------------------
% Loop through all IN-side mesh files
% For each file:
%   1. Read coordinates
%   2. Extract geometry derivatives
%   3. Extract GLL weights
%   4. Extract material parameters
%   5. Select boundary nodes matching OUT_MESH surfaces
% ------------------------------------------------------------------------
for iname = 1:length(filesx)
    % Read x-coordinates
    fileNamex = filesx(iname).name;
    ...
    %% ------------------------------------------------------------
    % Read Jacobian determinant (not the surface Jacobian!)
    % ------------------------------------------------------------
    ...
    fclose(fileId);
     ...
     %% ------------------------------------------------------------
    % Read inverse-Jacobian components:
    %   xixl = ∂ξ/∂x,  xiyl = ∂ξ/∂y,  xizl = ∂ξ/∂z
    %   etaxl = ∂η/∂x, ...
    %   gammaxl = ∂γ/∂x, ...
    %
    % NOTE:
    % SPECFEM3D_Cartesian stores ∂(ξ,η,γ)/∂(x,y,z), NOT ∂(x,y,z)/∂(ξ,η,γ).
    % We will invert them later when needed.
    % ------------------------------------------------------------
    % ----- xi derivatives -----
    ...
    fclose(fileId);
    %% ------------------------------------------------------------
    % Read GLL quadrature weights wx, wy, wz
    % ------------------------------------------------------------
    %wxgll
    ...
    fclose(fileId);    
    %% ------------------------------------------------------------
    % Read material parameters: kappa, mu, rho
    % ------------------------------------------------------------
    %kappa
    ...
    %% ------------------------------------------------------------
    % Select boundary nodes (IN-side) matching the 5 boundary faces
    % ------------------------------------------------------------
    ...
    % Filter them with bounding constraints (same as OUT-side)
    ...
    %% ------------------------------------------------------------
    % Append coordinate lists for later matching
    % ------------------------------------------------------------
...
%% ------------------------------------------------------------------------
% Match IN-side and OUT-side boundary coordinate sets
%
% OUT-side boundary points:
%       points1 = [X_all_2_check, Y_all_2_check, Z_all_2_check]
%
% IN-side boundary points:
%       points2 = [X_all_IN_check, Y_all_IN_check, Z_all_IN_check]
%
% The order of points is not guaranteed to be identical between
% OUT_MESH and IN_MESH, so we must:
%
%   1. Remove duplicate coordinates (stable ordering)
%   2. Match each OUT-side coordinate to the corresponding IN-side node
%   3. Reorder the IN-side arrays to match OUT-side ordering
% ------------------------------------------------------------------------
disp('check isequal:')
isequal(X_all_2_check,X_all_IN_check) 

%% ------------------------------------------------------------------------
% Assemble OUT-side points matrix
% ------------------------------------------------------------------------
points1 = [X_all_2_check, Y_all_2_check, Z_all_2_check];

% Find unique rows and the first index of each unique point
[uniquePoints, ~, idxUnique] = unique(points1, 'rows', 'stable');
[~, selectIdx] = ismember(uniquePoints, points1, 'rows');

% Keep only unique coordinates in original encounter order
points1 = points1(selectIdx,:);

%% ------------------------------------------------------------------------
% Assemble IN-side point matrix
% ------------------------------------------------------------------------
points2 = [X_all_IN_check, Y_all_IN_check, Z_all_IN_check]; % 第二组三维点

%% ------------------------------------------------------------------------
% Use ismember to find correspondence between the OUT-side points (points1)
% and IN-side points (points2).
%
% ------------------------------------------------------------------------
[~, reorderIdx] = ismember(points1, points2, 'rows');

%% ------------------------------------------------------------------------
% Check if any points failed to match
% Note: If the process runs correctly, all points will match correctly and no warnings will appear.
% ------------------------------------------------------------------------
if any(reorderIdx == 0)
    warning('Some boundary nodes in OUT_MESH were not found in IN_MESH.');
end

%% ------------------------------------------------------------------------
% Reorder IN-side coordinates so they match points1 exactly
% ------------------------------------------------------------------------
points2_reordered = points2(reorderIdx, :);

% double check: IN and OUT coordinate lists must match exactly
...
% Distance residual (should be zero)
dist_now = sqrt( (points1(:,1)-points2_reordered(:,1)).^2  + (points1(:,2)-points2_reordered(:,2)).^2  + (points1(:,3)-points2_reordered(:,3)).^2);


%% ------------------------------------------------------------------------
% Apply reorderIdx to all material, geometry, and weight arrays
% (so they match OUT-side boundary points ordering)
% ------------------------------------------------------------------------
...
%% ------------------------------------------------------------------------
% Load IN-side SGT matrices (source → boundary)
% These are stored in:
%   savedir_now_IN / proc*_SGT.mat
%
% Each .mat file typically contains:
%   Ivx, Ivy, Ivz     – displacement Green's tensors
%   Iv1x1 ... Iv3x3   – spatial derivatives of SGTs
%
% We group all MPI-rank results horizontally.
% After regroup, we reorder columns using reorderIdx so that
% SGTs correspond to the reordered boundary node list.
% ------------------------------------------------------------------------
disp('load SGT mat')
...
%% ------------------------------------------------------------------------
% Reorder IN-side SGT kernels to match IN→OUT node alignment
% ------------------------------------------------------------------------
Iux = Iux(:,reorderIdx);
Iuy = Iuy(:,reorderIdx);
Iuz = Iuz(:,reorderIdx);
...
%% ========================================================================
% Load OUT-side SGT matrices (receiver → boundary)
%
% Loaded from:
%   savedir_now_OUT / proc*_SGT.mat
%
% ========================================================================
...
%% ------------------------------------------------------------------------
% OUT-side reordering
% selectIdx corresponds to OUT-MESH's unique indexing
% ------------------------------------------------------------------------
Oux = Oux(:,selectIdx);
Ouy = Ouy(:,selectIdx);
Ouz = Ouz(:,selectIdx);
...
%
% SPECFEM outputs SGT ∂G/∂t (velocity Green's functions).
% The representation theorem requires displacement G(t).
% Because the displacement representation theorem states that
% displacement is obtained by convolving two displacements, but what
% we store is velocity. The acceleration is obtained by convolving two
% velocities. We must integrate one of them, and the velocity is obtained
% by convolving displacement and velocity.
% So:
% ========================================================================
...
%% ------------------------------------------------------------------------
% Construct boundary faces and evaluate the Representation Theorem 
%
% For each boundary face (xmin, xmax, ymin, ymax, zbottom):
%
%   1. Identify indices belonging to that face
%   2. Assign outward-pointing normal vector n = (n1, n2, n3)
%   3. For each node on that face, extract:
%        - IN-side SGTs:  U  (u, u1x1, u1x2, ...)
%        - OUT-side SGTs: G  (G, G1x1, G1x2, ...)
%        - Material constants λ, μ
%   4. Compute traction-like terms:
%
%        Term 2:   G  • (λ div(u) n + 2μ ε(u) n)
%        Term 3:   u  • (λ div(G) n + 2μ ε(G) n)
%
%   5. Compute the convolution U(t) = Term2(t) – Term3(t)
%   6. Store per-face results to face_results struct
%
% ------------------------------------------------------------------------

% First, locate boundary nodes on points1 (OUT-ordering)
NP = size(Ou3x3,2);
disp(NP);
...
% Change it into column vectors for safe
Bxixl_all = xixl_all(:);
Bxiyl_all = xiyl_all(:);
Bxizl_all = xizl_all(:);
...
for f = 1:length(face_names)
    ...
    % ------------------------------------------------------------
    % Construct outward-pointing normal vector for all nodes
    % ------------------------------------------------------------
    % For each node, Nlst1(id,:) = (n1, n2, n3)
    % Orientation: outward from the COMPUTATIONAL domain
    % ------------------------------------------------------------
    switch face_name
    ...
    %% ------------------------------------------------------------
    % Allocate arrays for RP terms on this face:
    %   U_RP_all        – final RP (term2 - term3)
    %   U_RP_term2_all  – contribution from term2
    %   U_RP_term3_all  – contribution from term3
    %
    % Each column corresponds to one boundary node.
    %
    % ------------------------------------------------------------
...
%%
    ...
    % Store results in face_results struct()
